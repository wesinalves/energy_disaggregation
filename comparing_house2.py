...
test.set_window(start="16-9-2013",end="30-9-2013") # to change to 5 house# to change to 5 house
...
top_5_train_elec = train_elec.submeters().select_using_appliances(type=['fridge', 'kettle', 'microwave','dish washer'])
...
def predict(clf, test_elec, sample_period, timezone):
    ...
    for i, chunk in enumerate(test_elec.mains().load(ac_type='active', sample_period=sample_period)):
        ...
        for meter in test_elec.submeters().select_using_appliances(type=['fridge', 'kettle', 'microwave','washing machine','dish washer']).meters:
