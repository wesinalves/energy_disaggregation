...
train_elec.append(elec1.submeters().select_using_appliances(type=['fridge', 'kettle', 'microwave','washing machine','dish washer']))

dataset2.set_window(start="2013-05-22",end="2013-10-03")
elec2 = dataset2.buildings[2].elec
train_elec.append(elec2.submeters().select_using_appliances(type=['fridge', 'kettle', 'microwave','dish washer']))
...
train_elec.append(elec4.submeters().select_using_appliances(type=['fridge', 'kettle']))

dataset5.set_window(start="2014-06-29",end="2014-09-01")
elec5 = dataset5.buildings[5].elec
train_elec.append(elec5.submeters().select_using_appliances(type=['washing machine']))
...
top_5_train_elec = nilmtk.global_meter_group.select_using_appliances(type=['fridge freezer', 'kettle', 'washer dryer','microwave','dish washer'])
...
def predict(clf, test_elec, sample_period, timezone):
    ...
    for i, chunk in enumerate(test_elec.mains().load(ac_type='active', sample_period=sample_period)):
        ...
        for meter in test_elec.submeters().select_using_appliances(type=['fridge', 'kettle','washing machine','microwave','dish washer']).meters:
